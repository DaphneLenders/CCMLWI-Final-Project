...
#        print("Not enough samples for author", author_name)
        ...
        augmenting_data_n = augmenting_data[~augmenting_data['Text'].isin(data_authors.loc[data_authors['Author'] == author_name]['Text'])]
        ...
        if(amount_samples_to_be_added>augmenting_data_n.shape[0]):
            amount_samples_to_be_added = augmenting_data_n.shape[0]
            print("   Not enough samples available to add, adding ",amount_samples_to_be_added," samples")
            # Now, augment our dataframe:
            data_authors_n = pd.concat([data_authors,augmenting_data_n]).reset_index(drop=True)
        else:   
            # Now, augment our dataframe:
...
def performClassification(data,leaveOuts,classifier,baseline_classifier,results_log_classifer,results_log_baseline):
...
#        train_featuresAll = new_features_train
        ...
        test_prediction_baseline = baseline_classifier.fit_and_predict(new_features_train, train_labelsALL, test_features)
        test_prediction = classifier.fit_and_predict(new_features_train, train_labelsALL, test_features)
...
###############################     MAIN STUFF    ##############################
# major stuff
## DONE: include fine-grained leaveouts
## DONE: balance datasets -> add actual author data (-> goal: have the same number everywhere)
## DONE: loop: add influencer data (%: +0,+25,+50,+75,+100)
## TODO: add second classifier

# minor stuff
# TOOD: rename function read_csv()
# TODO: overview file (-> table?)
...
#leaveOuts = ['lexicaldiversity','specialchars','biandtrigrams','postags','fasttext','sentiments','None']
...
amount_of_data_needed = [1.25,1.5,1.75,2]
...
for ratio in amount_of_data_needed:
    ...
    amount_samples_per_author_plus_influencer = int(amount_samples_per_author*ratio)  
    ...
    data_authors_plus_influencer = get_exactly_n_samples_for_given_author(data_authors, 'HPL', amount_samples_per_author_plus_influencer,"Final Influencers/Lovecraft_influencer/Lovecraft_influencer.txt")
    ...
    performClassification(data_authors_plus_influencer,leaveOuts, Classifier_XGBoost, Classifier_NaiveBayes,results_log_classifier,results_log_baseline)  
...
# Upon testing, we found out that the ratio that increased the classifier performance
# the most was 
...
# old stuff -> to be deleted by justine later, maybe she needs some of the ideas from this old code later :P 


#r=extract_features(train_set['Text'][0],'None')
#    , "sentiments":list(range(73,75)),"fasttext":list(range(75,375)), "None":[]}
#    sentence_tokenizer = nltk.data.load('tokenizers/punkt/english.pickle')
#    word_tokenizer = nltk.tokenize.RegexpTokenizer(r'\w+')
#    # bag_of_words_and_characters includes characters
#    bag_of_words_and_characters = [x for x in wordpunct_tokenize(text)]
...
##            print("Beginning of r lexicalsyntactic ", len(features))
#            # Count the number of words_only
#            features.append(len(bag_of_words_and_characters))
#            # Count the number of words_only, excluded the stopwords
#            features.append(len([x for x in bag_of_words_and_characters if x.lower() not in stop_words]))
...
#author_name = 'HPL'
#amount_samples_per_author =7500
#filename = "Influencer Texts/Lovecraft.txt"
#
#if(data_authors[data_authors['Author']==author_name].shape[0]>amount_samples_per_author):
#    print("Too many samples for author", author_name)
#    # Calculate how many samples we have to delete 
...
#    # Randomly select which indices have to be delted
#    indices_for_deletion = np.random.choice(data_authors[data_authors['Author']==author_name].index, amount_samples_to_be_deleted, replace=False)
#    data_authors = data_authors.drop(indices_for_deletion)  
## Case 2: We have to augment the data
#elif(data_authors[data_authors['Author']==author_name].shape[0]<amount_samples_per_author):
#    print("Not enough samples for author", author_name)
#    # Read in the additional data to a dataframe
...
#        # Now, augment our dataframe:
#        data_authors_n = pd.concat([data_authors,augmenting_data.sample(amount_samples_to_be_added)]).reset_index(drop=True)
#print("   Now, the amount of text samples for the author HPL is: ", data_authors_n[data_authors_n['Author']=='HPL'].shape[0]) 

## Compute tf-idf values
#print("    Preparing additional resources ...")
#tokenize = lambda doc: doc.lower().split(" ")
#sklearn_tfidf = TfidfVectorizer(norm='l2',min_df=0, use_idf=True, smooth_idf=False, sublinear_tf=True, tokenizer=tokenize)
#all_documents = [re.sub('[^A-Za-z\s]+', '', text).lower() for text in data_authors["Text"].tolist()]
#sklearn_representation = sklearn_tfidf.fit_transform(all_documents)    
#print("   Finished preparing additional resources")
#
#
...
#
## We want to store all important outputs of the console in a corresponding txt file
#filename = "results_log_influencer_data.txt" # TODO: dynamic
#results_log = open(filename,'w')
#
##results_file = open("kacka.txt","w")
#
#performClassification(data_authors,leaveOuts,Classifier_SVM,results_log)
#
#results_log.close()
#
#
...
### Regex was found here: https://stackoverflow.com/questions/25735644/python-regex-for-splitting-text-into-sentences-sentence-tokenizing
#sentences = re.split(r'(?<!\w\.\w.)(?<![A-Z][a-z]\.)(?<=\.|\?)\s', text)
#with open(filename,encoding = "ISO-8859-1") as f:
#    text = " ".join(f.read().split())

#data_authors = pd.DataFrame(data = {'Text' : ["The thing has gone for ever, Armitage said.", "k", "a", "m", "n"], 'Author' : ['HPL','HPL','HPL','EAP','HPL']}) 
#data_influencer = pd.DataFrame(data = {'Text' : ["Herbert West needed fresh bodies because his life work was the reanimation of the dead.", "Here we barricaded ourselves, and, for the present were secure.", "m"], 'Author' : ['HPL','HPL','HPL']})
...
#print("After check:")
#print(augmenting_data.shape)  
## Now, we can randomly add samples 
## Caution: We cannot sample more than the actual number of available additional sentences
#if(amount_samples_to_be_added>augmenting_data.shape[0]):
#    amount_samples_to_be_added = augmenting_data.shape[0]
## Now, augment our dataframe:
#kack = pd.concat([data_authors,augmenting_data.sample(amount_samples_to_be_added)]).reset_index(drop=True)
#    # Case 1: We have too many samples in -> randomly delete those that are too much
#    if(data_authors[data_authors['Author']==author_name].shape[0]>amount_samples_per_author):
#        print("Too many samples for author", author_name)
#        # Calculate how many samples we have to delete 
...
#            print("   Not enough samples available to add, adding ",amount_samples_to_be_added," samples")
#            # Now, augment our dataframe:
#            data_authors = pd.concat([data_authors,augmenting_data]).reset_index(drop=True)
#        else:   
#            # Now, augment our dataframe:
#            data_authors = pd.concat([data_authors,augmenting_data.sample(amount_samples_to_be_added)]).reset_index(drop=True)
##        # Now, augment our dataframe:
##        data_authors = pd.concat([data_authors,augmenting_data.sample(amount_samples_to_be_added)]).reset_index(drop=True)
#    return data_authors    
